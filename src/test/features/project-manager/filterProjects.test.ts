...
import { filterProjects } from "@/features/project-manager/utils/filterProjects";
...
describe("filterProjects", () => {
  ...
  it("filters by search term alone", () => {
    const items = [
      ...
      makeItem({ projectId: "beta", name: "Beta" }),
    ];

    const filtered = filterProjects(items, DEFAULT_FILTERS, "alp");
    console.log('filtered ids', filtered.map((item) => item.projectId));
    expect(filtered).toHaveLength(1);
