 ...
 */
function calculateSidebarOneWidth(state: LayoutState["sidebarOne"]): number {
  if (!state.mounted) {
    return 0;
  }
  return state.width;
 ...
 */
function calculateSidebarTwoWidth(state: LayoutState["sidebarTwo"]): number {
...
export function LayoutShell({ children }: PropsWithChildren) {
  ...
  // The grid template arranges three rows (header, main, footer) and four
  // columns (sidebarOne + sidebarTwo + sidemenu + main content). We translate runtime state into CSS so
  // transitions and conditional mounting stay in sync automatically.
  const templateRows = `${headerHeight}px 1fr ${footerHeight}px`;
  const templateColumns = `${sidebarOneWidth}px ${sidebarTwoWidth}px ${sidebarWidth}px minmax(0, 1fr)`;
