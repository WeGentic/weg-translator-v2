...
export function DropZone({
  ...
  const getContent = () => {
    ...
    const iconSize = variant === "empty-state" ? "h-12 w-12" : variant === "compact" ? "h-6 w-6" : "h-8 w-8";
    const titleSize = variant === "empty-state" ? "text-lg" : "text-base";
    const descSize = variant === "empty-state" ? "text-base" : "text-sm";
      ...
      <div className="flex flex-col items-center justify-center text-center space-y-4">
        ...
        <div className="space-y-2">
          <h3 className={cn("font-semibold text-foreground", titleSize)}>
            {title || (isDragActive ? "Drop files here" : "Drag files here")}
          </h3>
  ...
  return (
    <div
      ...
      role="button"
      aria-label={disabled ? "File drop zone (disabled)" : "Drop files here or click to browse"}
      onKeyDown={(e) => {
