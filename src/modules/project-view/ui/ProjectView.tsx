...
/**
 * Renders the project workspace surface and coordinates long-running actions
 * such as conversion plans, incremental file updates, and streaming status.
 */
export function ProjectView({ projectSummary }: Props) {
  ...
  /**
   * Kicks off the optimistic processing ticker so users see immediate feedback
   * while the conversion pipeline runs in the background.
   */
  ...
  /**
   * Advances the progress overlay without regressing previously shown steps.
   */
  ...
  /**
   * Smoothly resets the processing overlay once conversions settle.
   */
  ...
  /**
   * Executes the OpenXLIFF conversion workflow for the provided plan and
   * returns the refreshed project summary used throughout the UI.
   */
