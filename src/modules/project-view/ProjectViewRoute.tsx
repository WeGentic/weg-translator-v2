...
import { PROJECT_FILE_DIALOG_FILTERS } from "@/modules/project-view/config";
...
} from "./routes/project-view-context";
import {
  ProjectViewContent,
  ProjectViewFallback,
  type ProjectViewWorkspaceProps,
} from "./ProjectViewContent";
import { ProjectViewHeader } from "./ProjectViewHeader";
import { ProjectViewToolbar } from "./ProjectViewToolbar";

export type ProjectViewRouteProps = ProjectViewContextValue;

/**
 * Hosts the project overview workspace and keeps the shared context aligned
 * with backend state while file mutations and conversions occur.
 */
export function ProjectViewRoute(initialProps: ProjectViewRouteProps) {
  ...
  /**
   * Re-fetches bundle + statistics in parallel so downstream consumers always
   * read a consistent snapshot after any mutation completes.
   */
  ...
  /**
   * Orchestrates end-to-end regeneration for the provided files, capturing
   * partial failures so the UI can report granular status.
   */
  ...
  const providerValue = useMemo<ProjectViewContextValue>(
    () => ({ projectId, summary, bundle, statistics }),
    [bundle, projectId, statistics, summary],
  );

  return (
    <ProjectViewProvider value={providerValue}>
      <ProjectViewRouteContent
        isBusy={isBusy}
        onAddFiles={() => {
          void handleAddFiles();
        }}
        onRemoveFile={(fileUuid) => {
          void handleRemoveFile(fileUuid);
        }}
        onChangeRole={(fileUuid, nextRole) => {
          void handleChangeRole(fileUuid, nextRole);
        }}
...
/**
 * Derives the view model for the overview page from the shared context and
 * wires user actions back to the mutation handlers provided by the route.
 */
function ProjectViewRouteContent({
  isBusy,
  onAddFiles,
  onRemoveFile,
  onChangeRole,
  ...
  const { summary, bundle, statistics } = useProjectViewContext();
  ...
  const headingId = "ProjectView-heading";
  const workspaceProps: ProjectViewWorkspaceProps = {
    headingId,
    summary,
    fileCount,
    languagePairs,
  ...
  return (
    ...
    >
      <ProjectViewContent {...workspaceProps} />
    </ProjectViewLayout>
  );
}
