-- Baseline schema extracted from the former initialise_schema bootstrap.
-- This migration intentionally retains IF NOT EXISTS guards so it can run
-- safely on legacy installations while still bootstrapping fresh databases.
...
CREATE INDEX IF NOT EXISTS idx_project_language_pairs_project
    ON project_language_pairs(project_uuid);

CREATE INDEX IF NOT EXISTS idx_project_files_project
    ON project_files(project_uuid);

CREATE INDEX IF NOT EXISTS idx_artifacts_project
    ON artifacts(project_uuid);

CREATE UNIQUE INDEX IF NOT EXISTS ux_artifacts_project_artifact
    ON artifacts(project_uuid, artifact_uuid);
